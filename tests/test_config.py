...
def test_dbfilename_fail(server_manager: ServerManager) -> None:
    server = server_manager.start_server()
    client = create_client(server.domain, server.port)

    client.send(commands.config_get("dbfilename"))

    assert client.recv(1024) == responses.config_param("dbfilename", None)
...
def test_dir_fail(server_manager: ServerManager) -> None:
    server = server_manager.start_server()
    client = create_client(server.domain, server.port)

    client.send(commands.config_get("dir"))

    assert client.recv(1024) == responses.config_param("dir", None)
